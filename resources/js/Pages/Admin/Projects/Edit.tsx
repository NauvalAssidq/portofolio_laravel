...
import { ArrowLeft, Loader2, Save, ImageIcon, Globe, Calendar, Tag as TagIcon, X, Trash2 } from "lucide-react";
...
export default function Edit({ auth, project }: Props) {
    const { data, setData, post, processing, errors, transform } = useForm({
        title: project.title || "",
        category: project.category || "",
        year: project.year || "",
        image: project.image as string | File | null,
        link: project.link || "",
    });
    ...
    return (
        <AdminLayout user={auth.user} title="Edit Project">
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                <h2 className="text-2xl font-serif text-gray-900 leading-tight">Edit Project</h2>
                                <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full font-bold">#{project.id}</span>
                            </div>
                            <p className="text-sm text-gray-500 hidden sm:block">Update details for {project.title}.</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                            <DestructiveAlert
                                ...
                                description="This action cannot be undone. This project will be permanently deleted."
                                actionLabel="Delete Project"
                                onConfirm={() => router.delete(route('admin.projects.destroy', project.id))}
                                trigger={
                                    <Button
                                        variant="ghost"
                                        type="button"
