...
import { ArrowLeft, Loader2, Save, ImageIcon, Sparkles, Tag as TagIcon, X, ArrowUpRight } from "lucide-react";
...
export default function Create({ auth }: { auth: any }) {
    ...
    useEffect(() => {
        // Parse comma separated string to array for visual preview
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        // We need to transform the data to include strict array for specialities
        // BUT useForm's post will automatically convert to FormData if it detects a File object in `data`.
        // However, `specialities` is array of strings. FormData handles arrays as `specialities[0]`, etc.
        // Inertia handles this automatically.
        // We just need to make sure `specialities` is in the data object or injected.

        // Let's use router.post explicitly if we want full control OR update data first.
        // Actually, easiest way with useForm is to just set 'specialities' in the data object?
        // But our data object has `specialitiesInput` (string).
        // Let's use the `transform` callback option of `post`.
    ...
    return (
        <AdminLayout user={auth.user} title="New Service">
            ...
            <form onSubmit={handleSubmit} className="max-w-6xl mx-auto pb-20">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                        ...
                        <div>
                            <h2 className="text-2xl font-serif text-gray-900 leading-tight">Add New Service</h2>
                            <p className="text-sm text-gray-500 hidden sm:block">Define a new service offering for your portfolio.</p>
                        </div>
                    </div>
